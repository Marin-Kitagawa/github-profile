...
/**
 * CSS styles for the NPM stats card.
 *
...
/** NPM "n" monogram icon (rendered as SVG text inside a tiny viewBox). */
const NPM_ICON = `<text x="1" y="12" font-family="monospace" font-size="13" font-weight="bold" fill="${NPM_RED}">n</text>`;

/** Download arrow icon (octicon-style). */
const DOWNLOAD_ICON = `
...
/**
 * @typedef {import('../fetchers/types').NpmData} NpmData
 */

/**
 * Renders the NPM Package Stats card.
 *
 * @param {NpmData} npmData NPM package statistics.
 * @param {Partial<{
 ...
 */
const renderNpmCard = (npmData, options = {}) => {
  const {
    totalPackages,
  ...
  const card = new Card({
    ...
    defaultTitle: "NPM Package Stats",
    width: CARD_WIDTH,
    height: CARD_HEIGHT,
    border_radius,
  ...
  card.setAccessibilityLabel({
    title: card.title,
    desc: `Total packages: ${totalPackages}, Monthly downloads: ${kFormatter(totalMonthlyDownloads)}, Top package: ${topPackageName}`,
  });
...
export { renderNpmCard };
export default renderNpmCard;
