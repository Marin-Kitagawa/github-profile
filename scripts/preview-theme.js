...
const FAIL_TEXT = `
  \rUnfortunately, your theme PR contains an error or does not adhere to our [theme guidelines](https://github.com/Marin-Kitagawa/github-profile/blob/master/CONTRIBUTING.md#themes-contribution). Please fix the issues below, and we will review your\
...
const THEME_CONTRIB_GUIDELINES = `
  \rHi, thanks for the theme contribution. Please read our theme [contribution guidelines](https://github.com/Marin-Kitagawa/github-profile/blob/master/CONTRIBUTING.md#themes-contribution).

  \r> [!WARNING]\
  \r> Keep in mind that we already have a vast collection of different themes. To keep their number manageable, we began to add only themes supported by the community. Your pull request with theme addition will be merged once we get enough positive feedback from the community in the form of thumbs up :+1: emojis (see [#1935](https://github.com/Marin-Kitagawa/github-profile/issues/1935#top-themes-prs)). We expect to see at least 10-15 thumbs up before making a decision to merge your pull request into the master branch. Remember that you can also support themes of other contributors that you liked to speed up their merge.
  ...
  \r> [!NOTE]\
  \r> Also, note that if this theme is exclusively for your personal use, then instead of adding it to our theme collection, you can use card [customization options](https://github.com/Marin-Kitagawa/github-profile#customization).
`;
const COLOR_PROPS = {
  title_color: 6,
  icon_color: 6,
...
const getGRSLink = (colors) => {
  const url = `https://github-profile.vercel.app/api?username=anuraghazra`;
